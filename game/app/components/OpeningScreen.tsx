    ...
    <div className={styles.helpBackdrop}>
      ...
      >
        <header className={styles.helpHeader}>
          <p className={styles.eyebrow}>VANGUARD / FIELD NOTES</p>
          <button type="button" onClick={onClose} className={styles.closeButton}>
            CLOSE <span aria-hidden="true">×</span>
          </button>
        </header>
        <div className={styles.helpBody} role="region" aria-label={panel === "controls" ? "Controls content" : "Story content"} tabIndex={0}>
          ...
          {panel === "controls" ? (
            ...
            <div className={styles.story}>
              <p className={styles.eyebrow}>THE YEAR 2847</p>
              <p>Humanity has spread across the stars. Thousands of colony worlds. A golden age of expansion.</p>
              <p className={styles.storyAccent}>Then The Signal arrived.</p>
              ...
              <p className={styles.storyAccent}>Whatever the cost.</p>
            </div>
          )}
        </div>
      </div>
...
export default function OpeningScreen({
  ...
  return (
    <main className={styles.opening} data-opening-screen aria-label="Sector Zero opening screen">
      ...
      <div className={styles.composition} inert={panel !== null}>
        <header className={styles.header}>
          <p className={styles.shipLabel}>
            <svg viewBox="0 0 24 38" fill="none" aria-hidden="true">
              <path d="M12 1v36M8 7v18l4 7 4-7V7M4 14v9l8 10 8-10v-9M10 4l2-3 2 3" />
            </svg>
            UEC VANGUARD
          </p>
          <button type="button" className={styles.soundButton} onClick={onToggleMute} aria-pressed={muted}>
            SOUND {muted ? "OFF" : "ON"}
        ...
        <div className={styles.content}>
          <h1 id="experience-gate-title" className={styles.wordmark} aria-label="SECTOR ZERO">
            <span>SECTOR</span>{" "}<span>ZERO</span>
          </h1>
          <p className={styles.subtitle}>THE LAST PILOT OF SECTOR ZERO</p>
          <div className={styles.rule} aria-hidden="true" />
          <p className={styles.premise}>The signal is calling. Choose your course.</p>
          <GalaxyExperienceGate ready={ready} hasGalaxyRun={hasGalaxyRun} onGalaxy={onGalaxy} onLegacy={onLegacy} />
          <div className={styles.insignia} aria-hidden="true"><span /></div>
          <nav className={styles.helpLinks} aria-label="Before you launch">
          ...
          {playerName && playerName !== "Guest" && <p className={styles.pilot}>PILOT / {playerName}</p>}
        </div>
      </div>
